...
export class MasterController {
  ...
  private initializeRoutes(): void {
    ...
    this.router.post(
      "/getCarDetailsOfScheduleDate",
      this.getCarDetailsOfScheduleDate
    );
  ...
  public updateWorkStatus = async (req: Request, res: Response) => {
    try {
      ...
      let startDay = moment([
        moment(lastDateOfTheMonth).get("year"),
        moment(lastDateOfTheMonth).get("month"),
        moment(lastDateOfTheMonth).get("date"),
      ]).add(1, "day");
      // let lastDay = moment(lastDateOfTheMonth.toISOString()).format("YYYY-MM-DD");
      let dateFrom = moment(lastDateOfTheMonth)
        .subtract(8, "d")
        .format("YYYY-MM-DD");
      let scheduleDate = moment(req.body.date).format("YYYY-MM-DD");
      const locationsList = await Workers.updateWorkStatus(req.body);
      if (
        moment(scheduleDate).isSameOrAfter(dateFrom) ||
        moment(scheduleDate).isSame(lastDateOfTheMonth)
      ) {
        const scheduleExtend = await Workers.ExtendScheduleDates(
          req.body,
          startDay
        );
        if (!scheduleExtend) {
    ...
    try {
      ...
      if (isExistCust.length == 0 && isExistVehicle.length == 0) {
        ...
        if (newCustomerId.insertId != undefined) {
          const vehicleAddRes = await Vehicles.addVehicle(
            req.body.custDetails,
            vehicleSchedule,
            newCustomerId.insertId,
            req.body
          );
        ...
        console.log(
          "customer or vehcile already exist",
          isExistCust,
          isExistVehicle
        );
  ...
  public editCustomer = async (req: Request, res: Response) => {
    try {
      ...
      const Customer = await Customers.editCustomer(
        req.body.cust_id,
        req.body.custDetails
      );
      // console.log('inserted customer_id', newCustomerId.insertId)
      if (Customer) {
        if (req.body.addedVehicles.length > 0) {
          ...
          if (isExistVehicle.length == 0) {
            // if (newCustomerId.insertId != undefined) {
            var vehicleAddRes = await Vehicles.addVehicle(
              req.body.custDetails,
              vehicleSchedule,
              req.body.cust_id,
              req.body
        ...
        if (req.body.editedVehicles.length > 0) {
          try {
            isEditVehicle = await Vehicles.editVehicle(
              req.body.editedVehicles,
              req.body.cust_id,
              req.body.custDetails
            );
          } catch (err) {
            res
              ...
              .json({
                status: false,
                result: { isEditVehicle: false, isRemoveVehicle: false },
              });
            return;
          }
        ...
        if (req.body.removedvehicles.length > 0) {
          var isRemoveVehicle = await Vehicles.removeVehicle(
            req.body.removedvehicles,
            req.body.cust_id,
            req.body.custDetails
          );
        }
        ...
        res
          .status(200)
          .json({ status: true, result: { isEditVehicle, isRemoveVehicle } });
        // }
      }
  ...
  public activateCustomer = async (req: Request, res: Response) => {
    ...
    try {
      const activateCustomer = await Customers.activateCustomer(
        id,
        user_name,
        activateDate
      );
